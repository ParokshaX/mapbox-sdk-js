...
var API_DIRECTIONS = '/directions/v5/{account}/{profile}/{encodedWaypoints}.json{?access_token,alternatives,geometries,overview,radiuses,steps,continue_straight,bearings,annotations,language,roundabout_exits,exclude,voice_instructions,voice_units}';
...
MapboxDirections.prototype.getDirections = function(waypoints, options, callback) {
  ...
  var allowedGeometries = ['polyline', 'geojson', 'polyline6'];
  ...
  var allowedAnnotations = ['duration', 'distance', 'speed', 'congestion'];
  ...
  if (typeof options.roundabout_exits !== 'undefined') {
    params.roundabout_exits = options.roundabout_exits;
  }
  ...
  if (typeof options.voice_instructions !== 'undefined') {
    invariant(typeof options.voice_instructions === 'boolean', 'voice_instructions option must be boolean');
    params.voice_instructions = options.voice_instructions;
  }

  if (typeof options.banner_instructions !== 'undefined') {
    invariant(typeof options.banner_instructions === 'boolean', 'banner_instructions option must be boolean');
    params.banner_instructions = options.banner_instructions;
  }

  if (typeof options.voice_units !== 'undefined') {
    params.voice_units = options.voice_units;
  }
